...
group_by_gene <- function(df, hom=F) {
  ...
  grouped <- account_for_multigene(grouped)
  return(grouped)
}
...
get_burden <- function(slice, hom) {
  ...
  af <- ac/an
  is_rare <- af <= 0.01 & af > 0 
  df <- data.frame(lof_all=sum(ac > 0), lof_rare=sum(is_rare, na.rm=T), gene=gene)
  if (hom) {
    homs <- rowSums(as.matrix(slice[,hom_cols]))
    df$hom_all=sum(homs > 0)
    df$hom_rare=sum(homs > 0 & is_rare, na.rm=T)
  }
  return(df) # df has three fields: "ac_all", "ac_rare", (or "hom"), and "gene"
}

account_for_multigene <- function(df) {
